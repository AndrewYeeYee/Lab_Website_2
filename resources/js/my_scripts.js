...
function loadStatsPage(){
	...
	for(row_counter =2; row_counter < table.rows.length; row_counter++){
		...
		var opp = table.rows[row_counter].cells[3].innerHTML;
		var win_or_loss = (parseInt(home)>parseInt(opp));
		if (win_or_loss){
...
function loadPlayersPage(){
	...
	for (i=0; i < players.length; i++){
		myInner += "<a class='dropdown-item' href='#' onclick='switchPlayers("+i+")'>"+players[i].name+"</a>";
	}
	dropdown.innerHTML = myInner;
}
...
function switchPlayers(playerNum){
	...
	avgp = players[playerNum].pass_yards/players[playerNum].games_played;
	avgr = players[playerNum].rushing_yards/players[playerNum].games_played;
	avgrec = players[playerNum].receiving_yards/players[playerNum].games_played;
	var ap = document.getElementById("avg_p_yards");
	ap.innerHTML = avgp.toFixed(0)
	var ar = document.getElementById("avg_r_yards");
	ar.innerHTML = avgr.toFixed(0)
	var arec = document.getElementById("avg_rec_yards");
	arec.innerHTML = avgrec.toFixed(2)
}
